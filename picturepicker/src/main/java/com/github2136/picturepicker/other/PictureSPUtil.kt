...
class PictureSPUtil private constructor(context: Context) {
    private var sp: SharedPreferences = context.getSharedPreferences("picturepicker", Context.MODE_PRIVATE)
    ...
    companion object {
        @Volatile
        var instance: PictureSPUtil? = null

        fun getInstance(context: Context): PictureSPUtil {
            if (instance == null) {
                synchronized(PictureSPUtil::class) {
                    if (instance == null) {
                        instance = PictureSPUtil(context)
                    }
                }
            }
            return instance!!
